...
Source0:        https://code.citrite.net/rest/archive/latest/projects/XSU/repos/%{name}/archive?at=%{shortcommit}&format=tar.gz&prefix=%{name}-%{commit}#/%{name}-%{shortcommit}.tar.gz
Source1:        https://repo.citrite.net:443/xs-local-contrib/pacemaker/nagios-agents-metadata-%{nagios_hash}.tar.gz
...
Requires:      xenserver-libqb > 0.17.0
BuildRequires: xenserver-libqb-devel > 0.17.0
...
BuildRequires: bison byacc flex help2man pkgconfig(dbus-1)
...
Provides:      xenserver-%{name}-libs-devel = %{version}-%{release}
...
%prep
%autosetup -a 1 -n %{name}-%{commit} -S git_am -p1
...
- CP-27385: add pacemaker to xenserver-specs
- CA-285620: Drop dependency on gnutls
